...
function approveModificationRequest($conn, $data, $doctor_id) {
    ...
    if (function_exists('send_appointment_notification')) {
        ...
        // Doctor confirmation (their own log) + clinic staff (admins/superadmins)
        send_appointment_notification($conn, $doctor_id,
            'Appointment rescheduled',
            "You approved a parent's reschedule request — appointment is now on $when.",
            intval($req['appointment_id']));
        if (function_exists('get_admin_user_ids')) {
            foreach (get_admin_user_ids($conn) as $aid) {
                if ((int) $aid === (int) $doctor_id) continue;
                send_appointment_notification($conn, (int) $aid,
                    'Appointment rescheduled',
                    "A parent-initiated reschedule for appointment #" . intval($req['appointment_id']) . " was approved. New time: $when.",
                    intval($req['appointment_id']));
            }
        }
    }
