                    ...
                    <td class="text-end">
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                ...
                                <?php else: ?>
                                    <li><a class="dropdown-item" href="#" onclick="openEditModal(<?= $u['id'] ?>); return false;"><i class="bi bi-pencil me-2"></i>Edit profile</a></li>
                                    <li><a class="dropdown-item" href="#" onclick="resetUserPassword(<?= $u['id'] ?>); return false;"><i class="bi bi-key me-2"></i>Reset password</a></li>
...
<div class="modal fade" id="resetPasswordModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-key me-2"></i>Temporary Password</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <p>The user's password has been reset. A copy was emailed to them, but you can also share this temporary password directly. They will be required to change it on their next login.</p>
                <div class="input-group">
                    <input type="text" class="form-control" id="resetPasswordValue" readonly>
                    <button class="btn btn-outline-secondary" type="button" onclick="copyTempPassword()"><i class="bi bi-clipboard"></i></button>
                </div>
                <small class="text-muted d-block mt-2">This password will not be shown again.</small>
            </div>
            <div class="modal-footer">
...
async function resetUserPassword(userId) {
    showConfirm('Reset this user\'s password? A temporary one will be generated and emailed to them.', async () => {
        const result = await apiRequest(`/superadmin/users/${userId}/reset-password`, 'POST', {});
        if (result.success) {
            const tempPwd = (result.data && result.data.temporary_password) || '';
            if (tempPwd) {
                document.getElementById('resetPasswordValue').value = tempPwd;
