...
class UserService
{
    ...
    /**
     * Reset a user's password (SUPERADMIN-only action).
     *
     * Generates a strong temporary password, persists the hash, flags the
     * account so the user must change it on next login, and emails the
     * new credentials. Returns the plaintext password so the superadmin
     * can hand it to the user out-of-band if needed.
     */
    public function resetUserPassword(int $userId, int $superadminId): array
    {
        if ($userId === $superadminId) {
            return ['success' => false, 'message' => "Use the change-password page to reset your own password."];
        }
        ...
        $this->activityLog->log(
            'PASSWORD_RESET_BY_SUPERADMIN',
            $superadminId,
            'user',
            $userId,
            "Password reset for {$target['email']} by superadmin"
        );

        if (!empty($target['email'])) {
            $notificationService = new NotificationService();
            $notificationService->sendAdminPasswordReset(
                (int) $target['id'],
                $target['email'],
                $target['first_name'] ?? '',
                $tempPassword
        ...
        return [
            'success' => true,
            'message' => 'Password reset. The temporary password has been emailed to the user.',
            'data' => ['temporary_password' => $tempPassword],
        ];
    }

    /**
     * Generate a strong temporary password (12 chars, mixed case, digit, symbol).
     */
