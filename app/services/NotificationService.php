...
class NotificationService
{
    ...
    /**
     * Send a temp-password email after a superadmin-triggered reset.
     */
    public function sendAdminPasswordReset(int $userId, string $email, string $firstName, string $tempPassword): void
    {
        $config = require dirname(__DIR__, 2) . '/config/app.php';
        $loginUrl = $config['url'] . '/login';
        $greeting = $firstName !== '' ? 'Hi ' . htmlspecialchars($firstName) . ',' : 'Hello,';

        $body = "
            <h2>Your Password Has Been Reset</h2>
            <p>{$greeting}</p>
            <p>An administrator has reset the password on your PediCare Clinic account. Use the temporary credentials below to log in, and you'll be asked to set a new password immediately.</p>
            <div style='background:#f5f5f5;padding:15px;border-radius:5px;margin:20px 0;'>
            ...
            <p style='color:#777;font-size:13px;'>If you did not expect this change, please contact the clinic immediately.</p>
        ";

        $this->sendEmail($email, 'Your PediCare Password Has Been Reset', $body);
        $this->sendInApp(
            $userId,
            'Password Reset by Administrator',
            'Your account password was reset. Please log in with the new temporary password and set a new one.',
            'SYSTEM'
        );
    }
