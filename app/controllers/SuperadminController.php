...
class SuperadminController extends Controller
{
     ...
     *
     * The new password is emailed to the user and surfaced to the
     * superadmin so they can pass it on out-of-band if needed.
     */
    public function resetUserPassword(int $id): void
    {
        $result = $this->userService->resetUserPassword($id, (int) $this->userId());
        $result['success']
    ...
    /**
     * Reschedule an appointment from the superadmin view.
     */
    public function editAppointment(int $id): void
    {
        ...
        $data = array_merge($validation['data'], [
            ...
            'duration' => $this->input('duration') ? (int) $this->input('duration') : null,
        ]);
        $data = array_filter($data, fn($v) => $v !== null && $v !== '');

        $appointmentService = new \App\Services\AppointmentService();
        $result = $appointmentService->editAppointment($id, $data, (int) $this->userId());
        $result['success'] ? Response::success(null, $result['message']) : Response::error($result['message']);
    }

    /**
     * Cancel an appointment from the superadmin view.
     */
    public function cancelAppointment(int $id): void
    {
        $reason = $this->input('reason', 'Cancelled by superadmin');
        $appointmentService = new \App\Services\AppointmentService();
        $result = $appointmentService->updateStatus($id, 'CANCELLED', (int) $this->userId(), $reason);
        $result['success'] ? Response::success(null, $result['message']) : Response::error($result['message']);
    }
