...
<body>
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <?php else: foreach ($all_patients as $p): ?>
                                    <tr class="hover:bg-pink-50 cursor-pointer transition" onclick="openPatientSummary(<?php echo (int) $p['id']; ?>)" title="View patient summary">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-primary hover:underline"><?php echo htmlspecialchars($p['first_name'] . ' ' . $p['last_name']); ?></div>
                                        </td>
                ...
                <form id="editUserForm" onsubmit="handleEditUser(event)" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">New Password <span class="text-xs text-gray-400 font-normal">(leave blank to keep the current one)</span></label>
                            <div class="relative">
                                <input type="password" id="editUserPassword" autocomplete="new-password" minlength="8" maxlength="128" class="w-full px-3 py-2 pr-20 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Set a new password for this user">
                                <button type="button" onclick="toggleEditUserPasswordVisibility()" class="absolute inset-y-0 right-2 my-auto h-7 px-2 text-xs text-gray-500 hover:text-gray-700">Show</button>
                            </div>
                            <p class="mt-1 text-xs text-gray-500">Min 8 chars with upper, lower, digit, and symbol. The user will be required to change it on next login.</p>
                        </div>
                        <div>
    ...
    <?php if ($is_superadmin): ?>
    <!-- Patient Summary Modal (SuperAdmin) -->
    <div id="patientSummaryModal" class="fixed inset-0 modal-backdrop hidden z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl max-w-4xl w-full mx-4 max-h-screen overflow-y-auto">
            <div class="p-6 md:p-8">
                <div class="flex justify-between items-start mb-6">
                    <div>
                        <h2 class="text-2xl font-inter font-bold text-gray-800 mb-1">Patient Summary</h2>
                        <p class="text-sm text-gray-600">Profile, history, appointments and vaccinations.</p>
                    </div>
                    <button type="button" onclick="closePatientSummary()" class="text-gray-400 hover:text-gray-600">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
                <div id="patientSummaryBody">
                    <div class="text-center py-12 text-gray-500">Loading…</div>
                </div>
                <div class="flex justify-end pt-6">
                    <button type="button" onclick="closePatientSummary()" class="px-4 py-2 border border-gray-300 rounded-lg font-medium hover:bg-gray-50">Close</button>
                </div>
            </div>
        </div>
    </div>
    ...
    function closeEditUserModal() {
        ...
        var pwdBtn = document.querySelector('button[onclick="toggleEditUserPasswordVisibility()"]');
        if (pwdBtn) pwdBtn.textContent = 'Show';
    }

    function toggleEditUserPasswordVisibility() {
        var pwd = document.getElementById('editUserPassword');
        var btn = document.querySelector('button[onclick="toggleEditUserPasswordVisibility()"]');
        if (!pwd) return;
        if (pwd.type === 'password') { pwd.type = 'text'; if (btn) btn.textContent = 'Hide'; }
        else { pwd.type = 'password'; if (btn) btn.textContent = 'Show'; }
    }
    ...
    function handleEditUser(e) {
        ...
        if (newPassword !== '') {
            // Client-side strength hint — the server enforces the real rules.
            if (newPassword.length < 8
                || !/[A-Z]/.test(newPassword)
                || !/[a-z]/.test(newPassword)
                || !/[0-9]/.test(newPassword)
                || !/[!@#$%^&*()_+\-=\[\]{}|;:,.<>?]/.test(newPassword)) {
                notify('Password must be at least 8 chars with upper, lower, digit, and symbol.', 'error');
                return;
            }
    ...
    <?php if ($is_superadmin): ?>
    // ============================================
    // SuperAdmin: Patient Summary (clickable rows)
    // ============================================
    function openPatientSummary(patientId) {
        var modal = document.getElementById('patientSummaryModal');
        var body = document.getElementById('patientSummaryBody');
        if (!modal || !body) return;
        body.innerHTML = '<div class="text-center py-12 text-gray-500">Loading…</div>';
        modal.classList.remove('hidden');

        var fd = new FormData();
        fd.append('action', 'get_patient_summary');
        fd.append('csrf_token', window.CSRF_TOKEN);
        ...
        fetch('admin-actions-secure.php', { method: 'POST', body: fd })
            ...
            .then(function(d){
                if (!d.success) {
                    body.innerHTML = '<div class="text-center py-8 text-red-600">' + (d.message || 'Failed to load patient.') + '</div>';
                    return;
                }
                body.innerHTML = renderPatientSummary(d.data);
            })
            .catch(function(){
                body.innerHTML = '<div class="text-center py-8 text-red-600">Network error.</div>';
            });
    }
    ...
    function _fmtTime(s) {
        ...
        if (parts.length < 2) return _esc(s);
    ...
    function _statusBadge(status) {
        ...
        var color = { 'COMPLETED':'#28a745','CONFIRMED':'#007bff','CANCELLED':'#dc3545',
                      'NO_SHOW':'#6c757d','IN_PROGRESS':'#ffc107' }[s] || '#FF6B9A';
        return '<span class="inline-block px-2 py-0.5 rounded text-xs font-semibold text-white" style="background:' + color + '">' + _esc(s || '—') + '</span>';
    }
    ...
    function renderPatientSummary(d) {
        ...
        var vaxs = d.vaccinations || [];
        var cons = d.consultations || [];
        var rxs = d.prescriptions || [];

        var ageText = (p.age !== null && p.age !== undefined) ? (p.age + ' yrs') : '—';
        var heightWeight = [];
        if (p.height) heightWeight.push(_esc(p.height) + ' cm');
        if (p.weight) heightWeight.push(_esc(p.weight) + ' kg');
        var hw = heightWeight.length ? heightWeight.join(' · ') : '—';

        var html = '';

        // Profile + parent
        html += '<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">';
        html += '<div class="bg-pink-50 rounded-lg p-4">';
        html += '<h3 class="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Patient</h3>';
        html += '<div class="text-lg font-bold text-gray-900">' + _esc((p.first_name || '') + ' ' + (p.last_name || '')) + '</div>';
        html += '<div class="text-sm text-gray-600 mt-1">DOB: ' + _fmtDate(p.date_of_birth) + ' (' + ageText + ')</div>';
        html += '<div class="text-sm text-gray-600">Gender: ' + _esc(p.gender || '—') + ' · Blood: ' + _esc(p.blood_type || '—') + '</div>';
        html += '<div class="text-sm text-gray-600">Measurements: ' + hw + '</div>';
        if (p.allergies) html += '<div class="text-sm text-red-700 mt-2"><strong>Allergies:</strong> ' + _esc(p.allergies) + '</div>';
        if (p.medical_conditions) html += '<div class="text-sm text-gray-700"><strong>Conditions:</strong> ' + _esc(p.medical_conditions) + '</div>';
        if (p.special_notes) html += '<div class="text-sm text-gray-700"><strong>Notes:</strong> ' + _esc(p.special_notes) + '</div>';
        html += '</div>';

        html += '<div class="bg-blue-50 rounded-lg p-4">';
        html += '<h3 class="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Parent / Guardian</h3>';
        html += '<div class="text-lg font-bold text-gray-900">' + _esc((parent.first_name || '') + ' ' + (parent.last_name || '')) + '</div>';
        if (parent.email) html += '<div class="text-sm text-gray-600 mt-1"><a class="text-blue-600 hover:underline" href="mailto:' + _esc(parent.email) + '">' + _esc(parent.email) + '</a></div>';
        if (parent.phone) html += '<div class="text-sm text-gray-600">' + _esc(parent.phone) + '</div>';
        html += '</div>';
        html += '</div>';

        // Stats row
        html += '<div class="grid grid-cols-4 gap-3 mb-6">';
        html += _statCard('Appointments', apts.length);
        html += _statCard('Vaccinations', vaxs.length);
        html += _statCard('Consultations', cons.length);
        html += _statCard('Prescriptions', rxs.length);
        html += '</div>';

        // Appointments
        html += '<div class="mb-6"><h3 class="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Recent Appointments</h3>';
        if (!apts.length) {
            html += '<p class="text-sm text-gray-500">No appointments on record.</p>';
        } else {
            html += '<div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50"><tr>'
                 + '<th class="px-3 py-2 text-left">Date</th><th class="px-3 py-2 text-left">Time</th>'
                 + '<th class="px-3 py-2 text-left">Doctor</th><th class="px-3 py-2 text-left">Type</th>'
                 + '<th class="px-3 py-2 text-left">Status</th></tr></thead><tbody class="divide-y">';
            apts.forEach(function(a){
                html += '<tr>'
                    + '<td class="px-3 py-2">' + _fmtDate(a.appointment_date) + '</td>'
                    + '<td class="px-3 py-2">' + _fmtTime(a.appointment_time) + '</td>'
                    + '<td class="px-3 py-2">Dr. ' + _esc((a.doctor_first_name || '') + ' ' + (a.doctor_last_name || '')) + '</td>'
                    + '<td class="px-3 py-2">' + _esc(a.type || '—') + '</td>'
                    + '<td class="px-3 py-2">' + _statusBadge(a.status) + '</td>'
                    + '</tr>';
            });
        ...
        // Vaccinations
        html += '<div class="mb-6"><h3 class="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Vaccinations</h3>';
        if (!vaxs.length) {
            html += '<p class="text-sm text-gray-500">No vaccination records.</p>';
        } else {
            html += '<div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50"><tr>'
                 + '<th class="px-3 py-2 text-left">Vaccine</th><th class="px-3 py-2 text-left">Dose</th>'
                 + '<th class="px-3 py-2 text-left">Given</th><th class="px-3 py-2 text-left">Next due</th>'
                 + '<th class="px-3 py-2 text-left">Status</th></tr></thead><tbody class="divide-y">';
            vaxs.forEach(function(v){
                html += '<tr>'
                    + '<td class="px-3 py-2">' + _esc(v.vaccine_name || '—') + '</td>'
                    + '<td class="px-3 py-2">' + _esc(v.dose_number || '—') + ' / ' + _esc(v.total_doses || '—') + '</td>'
                    + '<td class="px-3 py-2">' + _fmtDate(v.administration_date) + '</td>'
                    + '<td class="px-3 py-2">' + _fmtDate(v.next_due_date) + '</td>'
                    + '<td class="px-3 py-2">' + _esc(v.status || '—') + '</td>'
                    + '</tr>';
            });
            html += '</tbody></table></div>';
        }
        ...
        if (cons.length) {
            html += '<div class="mb-6"><h3 class="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Recent Consultations</h3>';
            html += '<ul class="space-y-2">';
            cons.forEach(function(c){
                html += '<li class="border-l-4 border-pink-300 bg-pink-50 px-3 py-2 rounded">'
                    + '<div class="text-sm font-semibold">' + _esc(c.diagnosis || 'Consultation') + '</div>'
                    + '<div class="text-xs text-gray-600">'
                    + _fmtDate(c.created_at) + ' · Dr. ' + _esc((c.doctor_first_name || '') + ' ' + (c.doctor_last_name || ''))
                    + '</div>'
                    + (c.notes ? '<div class="text-sm text-gray-700 mt-1">' + _esc(c.notes) + '</div>' : '')
                    + '</li>';
            });
            html += '</ul></div>';
        }
        ...
        if (rxs.length) {
            html += '<div class="mb-2"><h3 class="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Recent Prescriptions</h3>';
            html += '<ul class="space-y-2">';
            rxs.forEach(function(r){
                html += '<li class="border-l-4 border-indigo-300 bg-indigo-50 px-3 py-2 rounded">'
                    + '<div class="text-sm font-semibold">' + _esc(r.diagnosis || 'Prescription') + '</div>'
                    + '<div class="text-xs text-gray-600">'
                    + _fmtDate(r.created_at) + ' · Dr. ' + _esc((r.doctor_first_name || '') + ' ' + (r.doctor_last_name || ''))
                    + '</div></li>';
            });
            html += '</ul></div>';
        }
    ...
    function _statCard(label, value) {
        return '<div class="bg-gray-50 rounded-lg p-3 text-center">'
             + '<div class="text-2xl font-bold text-gray-900">' + _esc(value) + '</div>'
             + '<div class="text-xs text-gray-600 uppercase tracking-wide">' + _esc(label) + '</div>'
             + '</div>';
    }
